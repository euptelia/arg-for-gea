...
parser.add_argument('-s', '--subsample',
                    help='1 for making a subsample and 0 for using all individuals',
                    type=int, default=0)
...
doSubsampling = bool(args.subsample)
...
outBasePath = "/home/tianlin/Documents/github/data/tskit_data/output/table/realistic_fpr_comparisons/250indSubset/"
...
if doSubsampling:
    model_name = model_name + "_subsample" + str(sample_size) + "individuals"
    ind_id = np.array(sorted(random.sample(range(ts.num_individuals),
                                           sample_size)))
    samples_id = list(np.stack((ind_id*2, ind_id*2+1)).ravel('F'))
    ts = ts.subset(samples_id)
...
# # Test: Extract frequency for subsamples
# freq_subsample = []
# num_samples = mts.num_samples
...
#### cor Freq-Env as an approximation of GEA
...
np.savetxt(outPath + model_name + "_withNeutralMut_mutSeed" +
           ...
           myTable,
           header="ID, allele_age, allele_freq, effect_size, pos, tau, tau-p, rho, rho-p")
